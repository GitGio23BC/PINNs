...
def create_graph(
    ...
    node_type = torch.zeros(mesh.n_nodes, dtype=torch.long, device=device)

    node_type[mesh.top_nodes] = 3
    node_type[mesh.bottom_nodes] = 3
    node_type[mesh.right_nodes] = 2
    node_type[mesh.left_nodes] = 1

    encoded_type = torch.nn.functional.one_hot(node_type, num_classes=4).float()

    if t is None:
        node_features = torch.cat([ref, cur, u, encoded_type], dim=-1)
    else:
        num_nodes = ref.shape[0]
        t_feat = t.view(1, 1).expand(num_nodes, 1)

        node_features = torch.cat([ref, cur, u, t_feat, encoded_type], dim=-1)
