...
def generate_ground_truth(
    ...
    S_applied_single = torch.stack(
        [torch.from_numpy(applied_stress_voigt(t)) for t in t_eval], dim=0
    )
    F_applied_trajectory = S_applied_single.unsqueeze(1).expand(-1, num_nodes, -1) * phys_cfg["area"]
    ...
    dataset = {
        ...
        "F_applied": F_applied_trajectory.to(device),
    }
