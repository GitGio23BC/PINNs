...
def test(model_path: Path | None = None):
    ...
    logger.info(f"Final Relative L2 Displacement Error: {final_l2_error:.6e}")
    ...
    base_u_norm_traj = [
        torch.linalg.norm(u[base_nodes_idx], dim=-1).mean().item() for u in all_u_pred
    ]
    tip_S11_pred_traj = [S[tip_nodes_idx, 0].mean().item() for S in all_S_pred]
    ...
    fig.suptitle(
        f"Model Evaluation: {model_path.name} | Relative L2 Error: {final_l2_error:.4e}",
        fontsize=13,
        fontweight="bold",
    )
    ...
    if metrics_path.exists():
        ...
        axes[0, 0].plot(df["total_loss"], "k-", label="Total Loss", linewidth=1.5)
